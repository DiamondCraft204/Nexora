/* =========================================================
   ...
   - Ganti Bahasa Indonesia/English (kamus ada di js/bahasa.js)
   ========================================================= */
...
// dipakai di semua tempat yang butuh mengganti/menambah ikon secara dinamis
// lewat DOM API (createElement), bukan lewat string HTML mentah.
function buatIkon(namaIkon, kelasTambahan) {
  const span = document.createElement('span');
  span.className = kelasTambahan
    ? 'material-symbols-outlined ' + kelasTambahan
...
/* =========================================================
   PENGATURAN ALAMAT — CUKUP UBAH DI BAGIAN INI
   ---------------------------------------------------------
   Semua alamat server dikumpulkan di sini. Kalau nama project atau domain
   Vercel-mu berubah, tidak perlu mencari-cari ke seluruh berkas.
   ========================================================= */

/* 1. ALAMAT PROJECT VERCEL (tempat produk.json dan folder api/ berada).
      Dipakai untuk: memuat produk & kupon, membuat gambar QRIS, dan
      menyinkronkan status pembayaran antar perangkat.
      Tulis TANPA garis miring di akhir. */
const NEXORA_ALAMAT_API = 'https://nexora-api-one.vercel.app';

/* 2. ALAMAT WEBSITE yang ditanam ke dalam kode QR.
      Kode QR berisi tautan halaman konfirmasi, dan tautan itu dibuka oleh HP
      LAIN — jadi alamatnya harus yang bisa dijangkau HP tersebut.

      Saat situs dijalankan lewat Live Server (localhost / 127.0.0.1) atau
      dibuka langsung sebagai berkas (file://), alamat itu HANYA berlaku di
      komputer ini: HP yang memindai akan menunjuk ke dirinya sendiri sehingga
      halaman gagal dibuka ("situs tidak dapat dijangkau").

      Pilihan isi:
        NEXORA_ALAMAT_API          -> website & API satu project Vercel (bawaan)
        'https://situsmu.vercel.app' -> website di-hosting terpisah
        'http://192.168.1.5:5500'  -> IP komputer di WiFi yang sama
                                      (http, BUKAN https — Live Server tidak
                                       melayani https; salah skema membuat HP
                                       gagal membuka halaman)
        ''                         -> ikuti alamat halaman yang sedang dibuka */
const NEXORA_BASIS_URL_PUBLIK = NEXORA_ALAMAT_API;

/* 3. SINKRONISASI PEMBAYARAN LINTAS PERANGKAT.
      Halaman checkout ada di laptop, halaman konfirmasi dibuka di HP. Keduanya
      browser berbeda sehingga tidak berbagi localStorage, jadi keduanya menitip
      pesan lewat endpoint /api/status-bayar pada NEXORA_ALAMAT_API di atas.

      Setel false kalau tidak memakai backend sama sekali — penyelesaian
      otomatis lalu hanya berlaku bila QR dibuka di browser yang sama. */
const NEXORA_SINKRON_BAYAR = true;
const NEXORA_JEDA_CEK_BAYAR = 3000; // jarak antar pengecekan ke server (ms)
...
// Masa berlaku satu kode QR (detik). Dipakai untuk hitung mundur di modal,
// ditanam sebagai waktu kedaluwarsa di dalam tautan hasil scan, dan dikirim ke
// backend supaya keduanya memakai angka yang sama persis.
const BERLAKU_DETIK_QRIS = 600;
...
async function muatDataProduk() {
  try {
    const response = await fetch(NEXORA_ALAMAT_API + '/produk.json');
    if (response.ok) {
...
function renderKatalog() {
  ...
  if (produkTersaring.length === 0) {
    ...
    pesanKosong.textContent = t('js.produkTidakDitemukan');
...
function buatKartuProduk(p) {
  ...
  spanTerjual.textContent = ambilTerjual(p) + ' ' + t('js.terjual');
...
function pasangModalKeDOM() {
  ...
  judulSpek.textContent = t('js.spekJudul');
  ...
  inputQty.addEventListener('input', function () {
    ...
    if (!isNaN(val) && val > stokTersedia) {
      ...
      tampilkanNotifikasi(t('js.maksBeliStok', { n: stokTersedia }), 'error');
    }
  });
  ...
  btnCart.replaceChildren(buatIkon('shopping_cart'), document.createTextNode(t('js.masukkanKeranjang')));
  btnCart.onclick = function () {
    const isLoggedIn = localStorage.getItem('isLoggedIn') === 'true';
    if (!isLoggedIn) {
      tampilkanNotifikasi(t('js.harusLogin'), 'error');
      setTimeout(function () {
    ...
    if (produkAktif) {
      ...
      if (sukses) {
        tutupModal();
        tampilkanNotifikasi(
          t('js.masukKeranjang', { nama: produkAktif.nama, n: qtyBeli }),
          'keranjang'
        );
      }
    }
  ...
  btnBuy.textContent = t('js.beliSekarang');
...
function bukaModal(p) {
  ...
  if (imgEl) {
    ...
    imgEl.alt = p.nama || t('js.produk');
  }
  const namaEl = document.getElementById('modalProdukNama');
  if (namaEl) namaEl.textContent = p.nama || t('js.produkTanpaNama');
  ...
  if (deskEl)
    deskEl.textContent = p.deskripsi || t('js.deskripsiDefault');
  const badgeTerjual = document.getElementById('modalProdukTerjual');
  if (badgeTerjual)
    badgeTerjual.replaceChildren(
      buatIkon('local_fire_department'),
      document.createTextNode(' ' + ambilTerjual(p) + ' ' + t('js.terjual'))
    );
  const badgeKategori = document.getElementById('modalProdukKategori');
  if (badgeKategori)
    badgeKategori.textContent = t('js.kategoriLabel') + (p.kategori || t('js.kategoriUmum')).toUpperCase();
  const stokVal = ambilStok(p);
  const stokEl = document.getElementById('modalProdukStok');
  if (stokEl) stokEl.textContent = t('js.tersisa', { n: stokVal });
  ...
  if (spekEl) {
    ...
    const labelMap = {
      bahan: t('js.spekBahan'),
      ukuran: t('js.spekUkuran'),
      berat: t('js.spekBerat'),
      kota: t('js.spekKota'),
      warna: t('js.spekWarna'),
      isi: t('js.spekIsi'),
      garansi: t('js.spekGaransi')
    };
    let adaSpek = false;
    ...
    if (!adaSpek) {
      const dt = document.createElement('dt');
      dt.textContent = t('js.spekKondisi');
      const dd = document.createElement('dd');
      dd.textContent = t('js.spekKondisiNilai');
      spekEl.appendChild(dt);
      spekEl.appendChild(dd);
    }
  }
...
function ubahJumlahModal(delta) {
  ...
  if (val > stokTersedia) {
    val = stokTersedia;
    tampilkanNotifikasi(t('js.batasStok', { n: stokTersedia }), 'error');
  }
  el.value = val;
}
...
function tambahKeKeranjang(idProduk, jumlah = 1) {
  ...
  if (stokTersedia <= 0) {
    tampilkanNotifikasi(t('js.stokHabis'), 'error');
    return false;
  }
  ...
  if (item) {
    if (item.qty + jumlah > stokTersedia) {
      tampilkanNotifikasi(t('js.stokKurang', { n: stokTersedia }), 'error');
      return false;
    }
    item.qty += jumlah;
  } else {
...
function ubahJumlahKeranjang(idProduk, jumlah) {
  ...
  if (jumlah <= 0) {
    ...
    if (jumlah > stokTersedia) {
      tampilkanNotifikasi(t('js.maksBeli', { n: stokTersedia }), 'error');
      return;
    }
    const item = daftar.find(function (i) {
      return String(i.id) === String(idProduk);
...
function renderKeranjang() {
  ...
  detail.forEach(function (item) {
    ...
    // Label "Harga satuan:" hanya muncul di tampilan HP (dirender CSS lewat
    // content: attr(data-label)), teksnya ikut bahasa yang aktif.
    pHarga.setAttribute('data-label', t('keranjang.labelHargaSatuan'));
    const stepper = document.createElement('div');
    ...
    btnHapus.textContent = t('js.hapus');
    btnHapus.onclick = function () {
      hapusDariKeranjang(item.id);
      delete statusCeklisKeranjang[item.id];
      renderKeranjang();
      tampilkanNotifikasi(t('js.barangDihapus'), 'error');
    };
...
function pasangEventKeranjang() {
  ...
  if (btnKosongkan) {
    btnKosongkan.onclick = function () {
      if (confirm(t('js.konfirmasiKosongkan'))) {
        kosongkanKeranjang();
        statusCeklisKeranjang = {};
        renderKeranjang();
        tampilkanNotifikasi(t('js.keranjangDikosongkan'), 'sukses');
      }
    };
  }
  ...
  if (btnCheckout) {
    btnCheckout.onclick = function () {
      ...
      if (terpilih.length === 0) return tampilkanNotifikasi(t('js.pilihMinimalSatu'), 'error');
...
function renderRiwayatPesanan() {
  ...
  if (semuaDaftar.length === 0) {
    ...
    p.textContent = t('js.riwayatKosong');
    wadah.appendChild(p);
  ...
  daftar.forEach(function (order) {
    ...
    strongNo.textContent = t('js.noPesanan');
    ...
    badge.textContent = t('js.statusSelesai');
    ...
    metode.textContent = t('js.metodeLabel') + order.metodeBayar;
    if (order.kodeKupon && order.diskon) {
      metode.textContent +=
        t('js.kuponLabel') + order.kodeKupon + ' (-' + formatRupiah(order.diskon) + ')';
    }
    const totalWrap = document.createElement('div');
    const textTotal = document.createTextNode(t('js.totalBelanja'));
...
function initProfile() {
  ...
  if (sbNama) sbNama.textContent = user.nama || t('profil.pengguna');
  ...
  if (btnEdit && btnSimpan) {
    ...
    formProfile.onsubmit = function (e) {
      ...
      tampilkanNotifikasi(t('js.profilDiperbarui'), 'sukses');
      inputs.forEach(function (el) {
  ...
  if (btnKeluar) {
    btnKeluar.onclick = function () {
      if (confirm(t('js.konfirmasiKeluar'))) {
  ...
  document.querySelectorAll('.btn-toggle-pwd').forEach(function (btn) {
    btn.onclick = function () {
      ...
      if (input.type === 'password') {
        input.type = 'text';
        this.replaceChildren(buatIkon('visibility_off'));
      } else {
        input.type = 'password';
        this.replaceChildren(buatIkon('visibility'));
      }
    };
  });
}
function initAuthForms() {
  ...
  // Cegah spasi diketik langsung di kolom Nama & Email (baik saat Daftar maupun Masuk)
  ['regNama', 'regEmail', 'loginEmail'].forEach(function (id) {
  ...
  if (formDaftar) {
    formDaftar.addEventListener('submit', function (e) {
      ...
      // Kolom "Umur" bersifat OPSIONAL pada form pendaftaran. Kalau elemennya
      // tidak ada di HTML, usia dihitung otomatis dari tanggal lahir. Tanpa
      // pengecekan ini, getElementById('regUmur') bernilai null dan membaca
      // .value langsung melempar error sehingga tombol Daftar terasa "mati".
      const elUmurDaftar = document.getElementById('regUmur');
      const umur = elUmurDaftar ? parseInt(elUmurDaftar.value, 10) : null;
      ...
      if (nama.length < 3) return tampilkanNotifikasi(t('js.namaMin3'), 'error');
      if (/\s/.test(nama)) return tampilkanNotifikasi(t('js.namaTanpaSpasi'), 'error');
      if (/\s/.test(email)) return tampilkanNotifikasi(t('js.emailTanpaSpasi'), 'error');
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
        return tampilkanNotifikasi(t('js.emailTidakValid'), 'error');
      if (password.length < 8)
        return tampilkanNotifikasi(t('js.sandiMin8'), 'error');
      if (!tanggalLahir) return tampilkanNotifikasi(t('js.tanggalWajib'), 'error');
      const tglInput = new Date(tanggalLahir);
      const tglHariIni = new Date();
      tglHariIni.setHours(0, 0, 0, 0);
      if (tglInput > tglHariIni)
        return tampilkanNotifikasi(t('js.tanggalMasaDepan'), 'error');
      const usiaSebenarnya = hitungUsiaDariTanggal(tanggalLahir);
      // Cocokkan umur dengan tanggal lahir hanya bila kolom umur memang ada
      if (elUmurDaftar && umur !== usiaSebenarnya) {
        return tampilkanNotifikasi(t('js.usiaTidakCocok', { n: umur }), 'error');
      }
      let users = JSON.parse(localStorage.getItem('nexoraUsers')) || [];
      ...
      if (emailSudahAda)
        return tampilkanNotifikasi(t('js.emailTerdaftar'), 'error');
      users.push({
        nama: nama,
        email: email,
        password: password,
        umur: elUmurDaftar ? umur : usiaSebenarnya,
        tanggalLahir: tanggalLahir,
        jenisKelamin: jenisKelamin
      });
      localStorage.setItem('nexoraUsers', JSON.stringify(users));
      tampilkanNotifikasi(t('js.daftarBerhasil'), 'sukses');
      formDaftar.reset();
  ...
  if (formMasuk) {
    formMasuk.addEventListener('submit', function (e) {
      ...
      if (!user) return tampilkanNotifikasi(t('js.emailBelumTerdaftar'), 'error');
      if (user.password !== password) return tampilkanNotifikasi(t('js.sandiSalah'), 'error');
      ...
      tampilkanNotifikasi(t('js.loginBerhasil'), 'sukses');
      setTimeout(function () {
...
function initAuthNav() {
  ...
  if (isLoggedIn) {
    greetAreas.forEach(function (greetArea) {
      greetArea.textContent = t('js.sapaan', {
        nama: nama ? nama.split(' ')[0] : t('profil.pengguna')
      });
    ...
    authLinks.forEach(function (authLink) {
      authLink.textContent = t('nav.keluar');
      authLink.href = '#';
...
function gantiTema() {
  ...
  tampilkanNotifikasi(temaBaru === 'dark' ? t('tema.notifGelap') : t('tema.notifTerang'), 'info');
}
function perbaruiIkonTema(tema) {
  document.querySelectorAll('.btn-toggle-theme').forEach(function (btn) {
    btn.replaceChildren(buatIkon(tema === 'dark' ? 'light_mode' : 'dark_mode'));
    btn.title = tema === 'dark' ? t('tema.keTerang') : t('tema.keGelap');
    // Label teks tombol tema di menu mobile (dirender CSS lewat content: attr(data-label))
    btn.setAttribute('data-label', t('tema.label'));
  });
}
...
function renderVoucherIndex() {
  ...
  KUPON_LIST.forEach(function (kupon) {
    ...
    if (ikonTipe[kupon.tipe]) {
      kiri.textContent = ikonTipe[kupon.tipe];
    } else if (kupon.tipe === 'ongkir') {
      kiri.replaceChildren(buatIkon('local_shipping'));
    } else {
      kiri.replaceChildren(buatIkon('confirmation_number'));
    }
    ...
    btnSalin.textContent = t('voucher.salin');
    btnSalin.onclick = function () {
      navigator.clipboard
        .writeText(kupon.kode)
        .then(function () {
          tampilkanNotifikasi(t('js.kodeDisalin', { kode: kupon.kode }), 'sukses');
        })
        .catch(function () {
          tampilkanNotifikasi(t('js.gagalSalin'), 'error');
        });
    };
...
/* ---------- ISI KODE QR: TAUTAN KE HALAMAN KONFIRMASI PEMBAYARAN ----------
   Dulu isi QR hanya berupa teks demo, jadi saat dipindai HP tidak terjadi
   apa-apa. Sekarang isi QR adalah ALAMAT halaman konfirmasi-bayar.html
   lengkap dengan ringkasan belanja yang dipadatkan ke dalam parameter ?d=,
   sehingga HP yang memindai langsung membuka halaman pembayaran berisi
   rincian pesanan + tombol "Pembayaran Selesai".

   Ringkasan sengaja diselipkan di URL (bukan diambil dari localStorage)
   karena HP yang memindai adalah perangkat/browser BERBEDA dari komputer
   tempat checkout dilakukan, sehingga tidak berbagi penyimpanan. */

const KUNCI_STATUS_BAYAR = 'nexora_status_bayar_';

// Encode/decode teks ke Base64 yang aman dipakai di URL (tanpa + / =)
function ubahKeBase64Url(teks) {
  const byte = new TextEncoder().encode(teks);
  let biner = '';
  byte.forEach(function (b) {
    biner += String.fromCharCode(b);
  });
  return btoa(biner).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
}
function ubahDariBase64Url(teks) {
  let b64 = String(teks).replace(/-/g, '+').replace(/_/g, '/');
  while (b64.length % 4 !== 0) b64 += '=';
  const biner = atob(b64);
  const byte = Uint8Array.from(biner, function (c) {
    return c.charCodeAt(0);
  });
  return new TextDecoder().decode(byte);
}

// Menyusun data ringkas transaksi. Nama kunci sengaja disingkat (r, t, i, ...)
// supaya URL tetap pendek dan kode QR mudah dipindai kamera HP.
function bangunDataQrisTransaksi(kodeRef, hitungan, daftarBarang, merchant, berlakuDetik) {
  return {
    v: 1,
    r: kodeRef,
    m: 'QRIS',
    ...
    o: hitungan.ongkir,
    d: hitungan.diskon,
    k: hitungan.kuponAktif ? hitungan.kuponAktif.kode : '',
    t: hitungan.total,
    e: Date.now() + (Number(berlakuDetik) || 600) * 1000,
    n: merchant.nama || NEXORA_MERCHANT_QRIS.nama,
    id: merchant.nmid || NEXORA_MERCHANT_QRIS.nmid,
    w: merchant.terhubungKe || NEXORA_MERCHANT_QRIS.terhubungKe,
    // Maksimal 15 produk ikut dititipkan supaya isi kode QR tidak kepanjangan
    // dan tetap mudah dipindai; sisanya cukup dihitung lewat kunci "x".
    i: (daftarBarang || []).slice(0, 15).map(function (item) {
      return [String(item.nama || '').slice(0, 45), Number(item.qty) || 1, Number(item.harga) || 0];
    }),
    x: Math.max(0, (daftarBarang || []).length - 15)
  };
}

// URL absolut halaman konfirmasi. Kalau NEXORA_BASIS_URL_PUBLIK diisi, alamat
// itu yang dipakai; kalau tidak, alamat halaman sekarang yang jadi basis
// (tetap benar baik lewat http://, https://, maupun berkas file://).
function bangunUrlKonfirmasiBayar(dataTransaksi) {
  const basis = String(NEXORA_BASIS_URL_PUBLIK || '').trim();
  const url = basis
    ? new URL('html/konfirmasi-bayar.html', basis.replace(/\/+$/, '') + '/')
    : new URL('konfirmasi-bayar.html', window.location.href);
  url.searchParams.set('d', ubahKeBase64Url(JSON.stringify(dataTransaksi)));
  return url.href;
}

// Memeriksa apakah tautan hasil scan benar-benar bisa dibuka dari perangkat lain.
// Mengembalikan KUNCI pesan peringatan, atau string kosong kalau tautannya sehat.
function masalahTautanQr(urlKonfirmasi) {
  let url;
  try {
    url = new URL(urlKonfirmasi);
  } catch (e) {
    return 'bayar.qrisPeringatanLokal';
  }

  // Berkas file:// dan alamat localhost hanya berlaku di komputer ini
  if (url.protocol !== 'http:' && url.protocol !== 'https:') return 'bayar.qrisPeringatanLokal';
  const host = url.hostname;
  if (host === '' || host === 'localhost' || host === '127.0.0.1' || host === '[::1]') {
    return 'bayar.qrisPeringatanLokal';
  }

  // Alamat IP jaringan lokal (192.168.x.x, 10.x.x.x, 172.16-31.x.x) yang memakai
  // https:// hampir pasti keliru — server pengembangan seperti Live Server atau
  // python http.server melayani http://, bukan https://. Kalau ini dibiarkan, HP
  // akan gagal dengan ERR_ADDRESS_UNREACHABLE / ERR_SSL_PROTOCOL_ERROR.
  const ipJaringanLokal = /^(10\.|192\.168\.|172\.(1[6-9]|2\d|3[01])\.)/.test(host);
  if (ipJaringanLokal && url.protocol === 'https:') return 'bayar.qrisPeringatanHttps';

  return '';
}

// Alamat gambar QR. Isinya = URL halaman konfirmasi di atas.
function bangunUrlGambarQris(kodeRef, isiQr) {
  return (
    'https://api.qrserver.com/v1/create-qr-code/?size=280x280&margin=6&color=' +
    pilihWarnaQr(kodeRef) +
    '&bgcolor=ffffff&data=' +
    encodeURIComponent(isiQr)
  );
}

/* ---------- MEMANTAU PEMBAYARAN DARI PERANGKAT LAIN ----------
   Saat tombol "Pembayaran Selesai" ditekan di halaman konfirmasi, halaman
   itu menuliskan penanda lunas ke localStorage. Kalau QR dipindai/dibuka di
   browser yang sama (mis. tab lain di perangkat yang sama), halaman checkout
   akan menangkap penanda tersebut dan menyelesaikan pesanan otomatis. */
let _pantauBayarInterval = null;
let _pantauBayarHandler = null;
let _pantauBayarServerInterval = null;
let _pantauBayarBatasWaktu = null;

function mulaiPantauPembayaranQr(kodeRef, saatLunas) {
  hentikanPantauPembayaranQr();
  const kunci = KUNCI_STATUS_BAYAR + kodeRef;
  let sudahSelesai = false;

  // Dipanggil begitu pembayaran terdeteksi lunas, dari sumber mana pun.
  // Dijaga agar hanya berjalan sekali walau dua sumber melapor bersamaan.
  const tuntas = function () {
  ...
  // (a) Penanda di localStorage — untuk QR yang dibuka di BROWSER YANG SAMA
  //     (mis. lewat tautan "Buka halaman pembayaran di perangkat ini").
  const cekLokal = function () {
    let nilai = null;
    try {
      nilai = localStorage.getItem(kunci);
    } catch (e) {
      return;
    }
    if (nilai) {
  ...
  _pantauBayarHandler = cekLokal;
  _pantauBayarInterval = setInterval(cekLokal, 1200);
  window.addEventListener('storage', cekLokal);

  // (b) Tanya server — untuk QR yang dipindai dari PERANGKAT LAIN (HP).
  //     Kegagalan jaringan sengaja didiamkan: pengecekan berikutnya mencoba lagi.
  if (NEXORA_SINKRON_BAYAR) {
    const cekServer = function () {
      fetch(NEXORA_ALAMAT_API + '/api/status-bayar?ref=' + encodeURIComponent(kodeRef), {
        cache: 'no-store'
      })
        .then(function (res) {
        ...
        .then(function (data) {
          if (data && data.lunas) tuntas();
        })
        .catch(function () {});
    };
    cekServer();
    _pantauBayarServerInterval = setInterval(cekServer, NEXORA_JEDA_CEK_BAYAR);

    // Berhenti sendiri setelah kode QR kedaluwarsa, supaya tidak menanyai
    // server terus-menerus kalau modal dibiarkan terbuka.
    _pantauBayarBatasWaktu = setTimeout(function () {
      clearInterval(_pantauBayarServerInterval);
      _pantauBayarServerInterval = null;
    }, (BERLAKU_DETIK_QRIS + 60) * 1000);
  }
}

function hentikanPantauPembayaranQr() {
  clearInterval(_pantauBayarInterval);
  _pantauBayarInterval = null;
  clearInterval(_pantauBayarServerInterval);
  _pantauBayarServerInterval = null;
  clearTimeout(_pantauBayarBatasWaktu);
  _pantauBayarBatasWaktu = null;
  if (_pantauBayarHandler) {
    window.removeEventListener('storage', _pantauBayarHandler);
    _pantauBayarHandler = null;
  }
}
...
// Meminta gambar QR yang digambar di backend (api/generate-qris pada project
// Vercel nexora-api-one). Field "isi" berisi TAUTAN halaman konfirmasi
// pembayaran — itulah yang digambar jadi kode QR, sehingga hasil pindaian
// langsung membuka halaman rincian belanja di HP.
async function buatGambarQrisDariServer(kodeRef, total, isiQr, berlakuDetik) {
  const response = await fetch(NEXORA_ALAMAT_API + '/api/generate-qris', {
    method: 'POST',
    headers: {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Terapkan filter kategori dari hash URL (dipakai saat klik kategori
  // dari halaman utama, contoh: bagianDalam.html#cat-pakaian), supaya
  // kategori bisa langsung dipakai tanpa harus login terlebih dahulu.
  if (window.location.hash) {
    const idKategoriDariHash = window.location.hash.substring(1);
    const radioKategoriDariHash = document.getElementById(idKategoriDariHash);
    if (radioKategoriDariHash && radioKategoriDariHash.name === 'filter-kategori') {
      radioKategoriDariHash.checked = true;
    }
  }

  inisialisasiTema();
  pasangTombolTema();
  NEXORA_BAHASA.pasangTombol();
  initAuthNav();
  initAuthForms();
  initProfile();
  perbaruiBadgeKeranjang();
  ...
  /* ---------- RENDER ULANG SAAT BAHASA DIGANTI ----------
     js/bahasa.js sudah mengurus semua teks statis yang bertanda data-i18n.
     Yang tersisa adalah konten yang dibuat oleh JavaScript (katalog, kartu
     keranjang, riwayat pesanan, voucher, sapaan pengguna), jadi bagian itu
     dirender ulang di sini setiap kali event 'bahasaBerubah' dikirim. */
  document.addEventListener('bahasaBerubah', function () {
    perbaruiIkonTema(document.documentElement.getAttribute('data-theme') || 'light');
    initAuthNav();
    renderKatalog();
    renderKeranjang();
    renderRingkasanPembayaran();
    ...
    // Nama pengguna di sidebar profil ditulis ulang karena teks bawaannya
    // ("Pengguna") ikut diterjemahkan oleh mesin bahasa.
    const sbNamaProfil = document.getElementById('sidebarNamaUser');
    const namaTersimpan = localStorage.getItem('namaLogin');
    if (sbNamaProfil && namaTersimpan) sbNamaProfil.textContent = namaTersimpan;
  });
  ...
  if (linkIkonKeranjang) {
    linkIkonKeranjang.addEventListener('click', function (e) {
      ...
      if (!isLoggedIn) {
        e.preventDefault();
        tampilkanNotifikasi(t('js.harusLoginKeranjang'), 'error');
        setTimeout(function () {
          window.location.href = NEXORA_PATHS.login;
        }, 1000);
      } else {
  ...
  if (formBayar) {
    ...
    const tautanQrisEl = document.getElementById('tautanQrisLanjutan');
    const peringatanQrLokalEl = document.getElementById('peringatanQrLokal');
    ...
    function tampilkanKuponAktifUI() {
      ...
      if (kupon) {
        ...
        if (user) {
          ...
          infoPemakaian = t('js.kuponDipakai', { n: sudahDipakai });
        }
        if (teksKuponAktif) {
          teksKuponAktif.replaceChildren(
            buatIkon('confirmation_number'),
            document.createTextNode(
              t('js.kuponDiterapkanInfo', { kode: kupon.kode }) +
    ...
    if (btnTerapkanKupon) {
      btnTerapkanKupon.onclick = function () {
        ...
        if (!kodeInput) return tampilkanNotifikasi(t('js.kuponIsiKode'), 'error');
        const kuponDitemukan = (KUPON_LIST || []).find(function (k) {
          return (k.kode || '').toUpperCase() === kodeInput;
        });
        if (!kuponDitemukan)
          return tampilkanNotifikasi(t('js.kuponTidakDitemukan'), 'error');
        ...
        if (user) {
          ...
          if (sudahDipakai >= BATAS_PEMAKAIAN_KUPON) {
            return tampilkanNotifikasi(
              t('js.kuponBatasPakai', {
                kode: kuponDitemukan.kode,
                n: BATAS_PEMAKAIAN_KUPON
              }),
              'error'
            );
          }
        ...
        if (kuponDitemukan.minBelanja && subtotal < kuponDitemukan.minBelanja) {
          return tampilkanNotifikasi(
            t('js.kuponMinBelanja', { nominal: formatRupiah(kuponDitemukan.minBelanja) }),
            'error'
          );
        }
        sessionStorage.setItem('kuponAktif', JSON.stringify(kuponDitemukan));
        tampilkanNotifikasi(t('js.kuponBerhasil', { kode: kuponDitemukan.kode }), 'sukses');
    ...
    if (btnHapusKupon) {
      btnHapusKupon.onclick = function () {
        ...
        tampilkanKuponAktifUI();
        renderRingkasanPembayaran();
        tampilkanNotifikasi(t('js.kuponDihapus'), 'sukses');
      };
    }
    tampilkanKuponAktifUI();
    ...
    formBayar.onsubmit = function (e) {
      ...
      if (nama.length < 3) return tampilkanNotifikasi(t('js.namaMin3Bayar'), 'error');
      if (alamat.length < 10)
        return tampilkanNotifikasi(t('js.alamatMin10'), 'error');
      if (!provinsi) return tampilkanNotifikasi(t('js.provinsiWajib'), 'error');
      if (!kota || !kecamatan)
        return tampilkanNotifikasi(t('js.kotaKecamatanWajib'), 'error');
      if (!/^[0-9]{5}$/.test(kodePos))
        return tampilkanNotifikasi(t('js.kodePos5'), 'error');
      ...
      if (metodeFinal === 'QRIS') {
        judulModalBayar.textContent = t('js.bayarViaQris');
        ...
        const hitunganQr = hitungTotalCheckoutSaatIni();
        const total = hitunganQr.total;
        ...
        // Barang yang sedang di-checkout, ikut dititipkan ke dalam kode QR
        let idQrTerpilih = [];
        try {
          idQrTerpilih = JSON.parse(sessionStorage.getItem('itemCheckout')) || [];
        } catch (errIdQr) {}
        const barangQr = ambilDetailKeranjang().filter(function (item) {
          return idQrTerpilih.includes(String(item.id));
        });

        // Data merchant dari produk.json (field "qris") yang sudah dimuat di awal
        const merchantQr =
          typeof DATA_QR !== 'undefined' && DATA_QR ? DATA_QR : NEXORA_MERCHANT_QRIS;
        const tulisMerchant = function (merchant) {
          if (qrisMerchantNamaEl)
            qrisMerchantNamaEl.textContent = merchant.nama || NEXORA_MERCHANT_QRIS.nama;
          if (qrisMerchantNmidEl)
            qrisMerchantNmidEl.textContent = merchant.nmid || NEXORA_MERCHANT_QRIS.nmid;
          if (qrisMerchantTerhubungEl)
            qrisMerchantTerhubungEl.textContent =
              merchant.terhubungKe || NEXORA_MERCHANT_QRIS.terhubungKe;
        };
        tulisMerchant(merchantQr);

        // Isi kode QR = tautan halaman konfirmasi + ringkasan belanja.
        // Tautan disusun di sini (bukan di server) karena waktu kedaluwarsanya
        // ikut ditanam di dalamnya dan harus sama dengan hitung mundur di modal.
        const dataTransaksi = bangunDataQrisTransaksi(
          kodeRef,
          hitunganQr,
          barangQr,
          merchantQr,
        ...
        const urlKonfirmasi = bangunUrlKonfirmasiBayar(dataTransaksi);
        if (tautanQrisEl) {
          tautanQrisEl.href = urlKonfirmasi;
          tautanQrisEl.hidden = false;
        }

        // Tampilkan peringatan kalau tautannya diperkirakan tidak bisa dibuka HP,
        // supaya kegagalan tidak terjadi diam-diam. Atribut data-i18n ikut diganti
        // agar pesannya tetap mengikuti bahasa yang aktif.
        if (peringatanQrLokalEl) {
          const kunciMasalah = masalahTautanQr(urlKonfirmasi);
          if (kunciMasalah) {
            peringatanQrLokalEl.dataset.i18n = kunciMasalah;
            peringatanQrLokalEl.textContent = t(kunciMasalah);
            peringatanQrLokalEl.hidden = false;
          } else {
            peringatanQrLokalEl.hidden = true;
          }
        }

        // Kosongkan gambar dulu selagi menunggu backend menggambar QR-nya
        gambarQrisLanjutan.removeAttribute('src');
        gambarQrisLanjutan.alt = t('js.membuatQr');
        mulaiCountdownQris(BERLAKU_DETIK_QRIS, qrisTimerEl);

        // Backend Vercel yang menggambar QR dari tautan di atas
        buatGambarQrisDariServer(kodeRef, total, urlKonfirmasi, BERLAKU_DETIK_QRIS)
          .then(function (dataQr) {
            gambarQrisLanjutan.src = dataQr.qrImage;
            gambarQrisLanjutan.alt = t('bayar.qrisAlt');
            if (dataQr.merchant) tulisMerchant(dataQr.merchant);
          })
          .catch(function (err) {
            // Cadangan darurat supaya checkout tidak macet saat backend bermasalah:
            // QR digambar lewat layanan publik, isinya tautan yang sama persis.
            console.warn('Backend QRIS tidak tersedia, memakai penggambar cadangan:', err);
            gambarQrisLanjutan.src = bangunUrlGambarQris(kodeRef, urlKonfirmasi);
            gambarQrisLanjutan.alt = t('bayar.qrisAlt');
          });

        // Pantau penanda "sudah dibayar" yang ditulis halaman konfirmasi
        mulaiPantauPembayaranQr(kodeRef, function () {
          clearInterval(_timerQrisInterval);
          modalLanjutan.hidden = true;
          tampilkanNotifikasi(t('js.qrTerbayar'), 'sukses');
          prosesSelesaiPesanan();
        });
      } else if (metodeFinal === 'DANA' || metodeFinal === 'GoPay') {
        judulModalBayar.textContent = t('js.bayarVia', { metode: metodeFinal });
        wadahQrisLanjutan.hidden = true;
        wadahEwalletLanjutan.hidden = false;
        inputHpEwallet.value = '';
        inputHpEwallet.placeholder = t('js.phEwalletNomor', { metode: metodeFinal });
        modalLanjutan.hidden = false;
      } else {
    ...
    document.getElementById('btnKonfirmasiBayar').onclick = function (e) {
      ...
      if (
        ...
        if (!ewalletNum) {
          return tampilkanNotifikasi(
            t('js.ewalletWajib', { metode: dataPembayaranSementara.metodeFinal }),
            'error'
          );
        }
    ...
    function prosesSelesaiPesanan() {
      ...
      const dataPesanan = {
        ...
        tanggal: new Date().toLocaleDateString(NEXORA_BAHASA.locale(), {
          day: 'numeric',
          month: 'short',
          year: 'numeric'
        }),
      ...
      if (overlay) {
        document.getElementById('teksNomorPesanan').textContent =
          t('js.nomorPesanan') + nomorOrder;
        overlay.hidden = false;
        tampilkanNotifikasi(t('js.bayarBerhasil'), 'sukses');
      }
    }
  }
});